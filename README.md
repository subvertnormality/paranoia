...
Native snapshot batching is being requalified under a [bounded contract](docs/snapshot-batching-requalification-plan.md). The [initial retained results](docs/snapshot-batching-requalification-results.md) show roughly 93% less setup time and approximately flat whole-call timings, but CODE review found acceptance gaps in full public-handler coverage, successful-stderr custody, and stale-bytecode admission. Those observations remain historical; corrected qualification and CODE convergence are required before delivery.
