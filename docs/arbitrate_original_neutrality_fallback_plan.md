...
1. Extend the attester contract with one closed verdict line:
   ...
   packet. A FAIL must name a known option or field and quote an exact non-empty substring from that
   original packet; a free-text or unknown target is invalid. Stakes and context
...
3. Bound the complete original framing before the first cleaner call: retain the decision, option,
   ...
   After parsing a cleaner candidate, enforce a bounded absolute candidate size before attestation.
...
5. Add a deterministic fallback after a complete, bounded candidate receives a valid attestation:
...
6. Use this route table on every attempt: malformed/unbounded/incomplete cleaner output retries
   without fallback because it cannot be safely attested; context or stakes advocacy blocks the run;
   a fully valid cleaned packet wins; otherwise an eligible neutral original falls back; otherwise
...
- Relative length-band rejection remains attested and can fall back; malformed IDs, missing blocks,
  and absolute oversize remain structural retry/failure paths.
